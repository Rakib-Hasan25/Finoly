...
import { Achievement } from "@/tracker-types";
...
export const calculateUserLevel = (points: number) => {
  ...
  for (let i = USER_LEVELS.length - 1; i >= 0; i--) {
    if (points >= USER_LEVELS[i].requiredPoints) {
      ...
      if (nextLevel) {
        progress =
          ((points - USER_LEVELS[i].requiredPoints) /
            (nextLevel.requiredPoints - USER_LEVELS[i].requiredPoints)) *
          100;
      } else {
        progress = 100;
      }
  ...
  return {
    ...
    requiredPoints: USER_LEVELS[currentLevel]
      ? USER_LEVELS[currentLevel].requiredPoints
      : USER_LEVELS[USER_LEVELS.length - 1].requiredPoints,
  };
};
...
export const checkAchievements = () => {
  ...
  if (
    ...
    !currentAchievements.find((a) => a.id === "first-expense")
  ) {
  ...
  if (
    ...
    underBudget &&
    !currentAchievements.find((a) => a.id === "budget-master")
  ) {
    newAchievements.push({
  ...
  if (
    ...
    !currentAchievements.find((a) => a.id === "savings-hero")
  ) {
    newAchievements.push({
  ...
  if (
    ...
    !currentAchievements.find((a) => a.id === "debt-slayer")
  ) {
    newAchievements.push({
