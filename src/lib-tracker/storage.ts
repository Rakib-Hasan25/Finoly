import { Transaction, Budget, SavingsGoal, BankAccount, Debt, Achievement } from '@/types';
...
export const storage = {
  getTransactions: (): Transaction[] => {
    ...
    try {
      ...
      return transactions.map((transaction: any) => ({
        ...transaction,
        amount: Number.isFinite(parseFloat(transaction.amount)) ? parseFloat(transaction.amount) : 0,
  ...
  getBudgets: (): Budget[] => {
    ...
    try {
      ...
      return budgets.map((budget: any) => ({
        ...budget,
        amount: Number.isFinite(parseFloat(budget.amount)) ? parseFloat(budget.amount) : 0,
        spent: Number.isFinite(parseFloat(budget.spent)) ? parseFloat(budget.spent) : 0
      }));
    } catch (error) {
  ...
  getSavingsGoals: (): SavingsGoal[] => {
    ...
    try {
      ...
      return goals.map((goal: any) => ({
        ...goal,
        targetAmount: Number.isFinite(parseFloat(goal.targetAmount)) ? parseFloat(goal.targetAmount) : 0,
        currentAmount: Number.isFinite(parseFloat(goal.currentAmount)) ? parseFloat(goal.currentAmount) : 0
      }));
    } catch (error) {
  ...
  getBankAccounts: (): BankAccount[] => {
    ...
    try {
      ...
      return accounts.map((account: any) => ({
        ...account,
        balance: Number.isFinite(parseFloat(account.balance)) ? parseFloat(account.balance) : 0
      }));
    } catch (error) {
  ...
  getDebts: (): Debt[] => {
    ...
    try {
      ...
      return debts.map((debt: any) => ({
        ...debt,
        amount: Number.isFinite(parseFloat(debt.amount)) ? parseFloat(debt.amount) : 0,
        monthlyPayment: Number.isFinite(parseFloat(debt.monthlyPayment)) ? parseFloat(debt.monthlyPayment) : 0,
        interestRate: Number.isFinite(parseFloat(debt.interestRate)) ? parseFloat(debt.interestRate) : 0,
        remainingMonths: Number.isFinite(parseInt(debt.remainingMonths)) ? parseInt(debt.remainingMonths) : 0
      }));
    } catch (error) {
  ...
  getUserLevel: () => {
    ...
    try {
      ...
      return {
        level: Number.isFinite(parseInt(levelData.level)) ? parseInt(levelData.level) : 1,
        points: Number.isFinite(parseInt(levelData.points)) ? parseInt(levelData.points) : 0
      };
    } catch (error) {
  ...
  getUserStreak: () => {
    ...
    try {
      const streak = parseInt(localStorage.getItem(STORAGE_KEYS.USER_STREAK) || '0');
